...
function Display_Stall() {
    s =
        'document.instruction_table.column' +
        current_cycle +
        '[' +
        x +
        "].value = 'S'; document.instruction_table.column" +
        current_cycle +
        '[' +
        x +
        "].className='stall_red';";
    ...
    if (number_of_rows != 1) {
            ...
            if (stall == 1) {
                ...
                switch (parent.top_frame.instruction_array[x].pipeline_stage) {
                    ...
                    case 'ID':
                        ...
                        for (i = x - 1; i >= 0; i--) {
                            ...
                            if (
                                parent.top_frame.instruction_array[i]
                                    .pipeline_stage == 'EX' &&
                                parent.top_frame.instruction_array[i].operator
                                    .functional_unit ==
                                    parent.top_frame.instruction_array[x]
                                        .operator.functional_unit
                            ) {
                                // *** Structural Hazard ***
                                stall = 1;
                                break;
                            ...
                            // registers of the current instruction.
                                ...
                                if (data_forwarding == 1) {
                                    ...
                                    if (
                                        ...
                                        if (
                                            parent.top_frame.instruction_array[
                                                i
                                            ].pipeline_stage != 'WB' &&
                                            parent.top_frame.instruction_array[
                                                i
                        ...
                        if (stall != 1) {
                            ...
                            if (
                                parent.top_frame.instruction_array[x].operator
                                    .name == 'br_taken'
                            ) {
                                parent.top_frame.instruction_array[
                                    x
                ...
                // If an instruction is in the EX stage, output the functional unit.
                    ...
                    s =
                        ...
                        '[' +
                        x +
                        '].value = parent.top_frame.instruction_array[x].operator.display_value;';
                }
                // Output the pipeline stage.
                    ...
                    s =
                        ...
                        '[' +
                        x +
                        '].value = parent.top_frame.instruction_array[x].pipeline_stage;';
                }
        ...
        if (current_cycle == 0) {
            ...
            s =
                'document.instruction_table.column' +
                current_cycle +
                '.value = parent.top_frame.instruction_array[0].pipeline_stage;';
            eval(s);
            current_cycle++;
        } else {
            switch (parent.top_frame.instruction_array[0].pipeline_stage) {
                ...
                case 'ID':
                    ...
                    if (
                        parent.top_frame.instruction_array[0].operator.name.substring(
                            0,
                            2
                        ) == 'br'
                    ) {
                ...
                case 'EX':
                    ...
                    if (
                        parent.top_frame.instruction_array[0].execute_counter <
                        parent.top_frame.instruction_array[0].operator
                            .execution_cycles
                    ) {
                        parent.top_frame.instruction_array[0].pipeline_stage =
                            'EX';
            ...
            if (parent.top_frame.instruction_array[0].pipeline_stage == 'EX') {
                s =
                    'document.instruction_table.column' +
                    current_cycle +
                    '.value = parent.top_frame.instruction_array[0].operator.display_value;';
            } else {
                s =
                    'document.instruction_table.column' +
                    current_cycle +
...
function Add_Dependency(input) {
    ...
    parent.top_frame.dependencies += `${input}\n`;
}
...
function Check_Depencency() {
    ...
    for (i = 0; i < number_of_rows; i++) {
        ...
        for (j = 0; j < i; j++) {
            ...
            if (
                parent.top_frame.instruction_array[j].destination_register ==
                    parent.top_frame.instruction_array[i].source_register1 ||
                parent.top_frame.instruction_array[j].destination_register ==
                    parent.top_frame.instruction_array[i].source_register2
            ) {
                // A load instruction will take one cycle longer than any other instruction when forwarding is enabled.
                if (
                    (parent.top_frame.instruction_array[j].operator.name ==
                        'ld' ||
                        parent.top_frame.instruction_array[j].operator.name ==
                            'ld') &&
                    data_forwarding == 1
                    ...
                    if (
                        i - j <
                        parent.top_frame.instruction_array[j].operator
                            .execution_cycles +
                            non_forward_delay +
                            1
                    ) {
                        dep_exists = 1;
                        const latchName = j === i - 1 ? 'L3' : 'L4';
                        Add_Dependency(
                            `RAW: Instruction ${i} has data forwarded from ${latchName}`
                        );
                    }
                } else {
                    // If the difference between when the instructions would be issued is greater than the number of execution cycles
            ...
            if (
                (parent.top_frame.instruction_array[j].source_register1 ==
                    parent.top_frame.instruction_array[i]
                        .destination_register ||
                    parent.top_frame.instruction_array[j].source_register2 ==
                        parent.top_frame.instruction_array[i]
                            .destination_register) &&
                i - j <
                    abs(
                ...
                const latchName = j === i - 1 ? 'L3' : 'L4';
                Add_Dependency(
                    `WAR: Instruction ${i} has data forwarded from ${latchName}`
                );
            }
            ...
            if (
                parent.top_frame.instruction_array[i].destination_register ==
                    parent.top_frame.instruction_array[j]
                        .destination_register &&
                i - j <=
                    abs(
                        parent.top_frame.instruction_array[j].operator
                ...
                if (
                    ...
                    const latchName = j === i - 1 ? 'L3' : 'L4';
                    Add_Dependency(
                        `WAW: Instruction ${i} has data forwarded from ${latchName}`
                    );
                }
            }
            ...
            if (
                ...
                const latchName = j === i - 1 ? 'L3' : 'L4';
                Add_Dependency(
                    `SH: Instruction ${i} has data forwarded from ${latchName}`
                );
            }
            ...
            if (
                ...
                Add_Dependency(
                    'Instructions ' +
                        ...
                        ' and ' +
                        i +
                        ' will enter the MEM and WB stage at the same time.'
                );
            }
        }
...
function BuildPipeline() {
    ...
    document.write(
        "<p><form name='instruction_table'><table border='1'><tr><td colspan='2'></td><td align='center' colspan='" +
            number_of_columns +
            "'><b>CPU Cycles</b></td></tr><tr><td colspan='2' align='center'><b>Instruction<b></td>"
    );
    ...
    for (x = 0; x < number_of_rows; x++) {
        ...
        if (
            parent.top_frame.instruction_array[x].destination_register == 'null'
        ) {
            // Store instructions must be handled in a special way.
            if (
                parent.top_frame.instruction_array[x].operator.name == 'sd' ||
                parent.top_frame.instruction_array[x].operator.name == 'sd'
            ) {
                document.write(
                    '<tr><td>&nbsp<b>' +
                        x +
                        "</b>&nbsp</td><td><input name='instruction' readonly='1' size='25' value='" +
                        parent.top_frame.instruction_array[x].operator.name +
                        ' (' +
                        parent.top_frame.instruction_array[x].source_register1 +
                        ', Offset, ' +
                        parent.top_frame.instruction_array[x].source_register2 +
                        ")'>"
                );
            } else {
                document.write(
            ...
            document.write(
                '<tr><td>&nbsp<b>' +
                    ...
                    parent.top_frame.instruction_array[x].operator.name +
                    ' (' +
                    parent.top_frame.instruction_array[x].destination_register +
                    ', ' +
                    parent.top_frame.instruction_array[x].source_register1 +
                    ', ' +
                    parent.top_frame.instruction_array[x].source_register2 +
        ...
        for (y = 0; y < number_of_columns; y++) {
            document.write(
                "<td><input name='column" + y + "' readonly='1' size='7'></td>"
            );
        }
    ...
    document.write(
        "</table><input type='button' value='Step' onClick='Step();'>&nbsp;<input type='button' value='Execute All Instructions' 	onClick='Step_To_End();'></form>"
    );
}
...
for (i = 0; i < number_of_rows; i++) {
    number_of_columns +=
        parent.top_frame.instruction_array[i].operator.execution_cycles + 4;
}
